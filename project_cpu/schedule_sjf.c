...
void add(char *name, int priority, int burst){
  ...
  //According to the Shortest-Job-First Scheduling principle, we put the Task in the order of Brust from smallest to largest/
   while((temp->next != NULL) && (newTask->burst >= temp->next->task-> burst)){
      temp = temp->next;
   }
...
int total_waiting_time()
{
  ...
  //The total waiting time of Shortest-Job-First Scheduling is same as First- Come First-Served//
  //The waiting time of P(n) is equal to the waiting time of P(n-1) plus its brust.//
...
//Because in Shortest-Job-First Scheduling, Task is run all the brust at once. //
